        ...
        <table className="w-full min-w-full">
          ...
          <tbody className="divide-y divide-border">
            {filteredData.map((row, rowIndex) => (
              <tr
                key={rowIndex}
                className={cn("transition-colors hover:bg-muted/50", rowIndex % 2 === 0 ? "bg-card" : "bg-card/80")}
              >
                {columns.map((column) => (
                  <td key={String(column.key)} className={cn("px-4 py-3 text-sm text-foreground", column.className)}>
        ...
        {filteredData.map((row, rowIndex) => (
          ...
          >
            {columns.map((column) => (
              <div
                key={String(column.key)}
                className={cn("mb-3 last:mb-0", column.key === columns[0].key && "border-b border-border pb-3 mb-3")}
              >
